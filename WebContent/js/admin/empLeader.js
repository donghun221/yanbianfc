...
$(document).ready(function()
{
	...
	fileUpload($("#empImage",$("#editModal")),EnvConfig.ATTACH_UPLOAD_URL,EnvConfig.ATTACH_PATH);//*****
});
		...
		success: function(response)
		{
			if (response.code == ResponseCode.Success)
			{
				...
				$.each(response.data, function(index, value)
				{
					...
					tr.append("<td>" + value.empName + "</td>");//*****
					tr.append("<td>" + value.empTitle + "</td>");//*****
					var imageShowBtn = $("<td>").append(getImageShowBtn(EnvConfig.ATTACH_PATH,value.empImage.fileName));
					...
					var editButton = $("<button class='btn btn-sm btn-default' title='修改'><i class='glyphicon glyphicon-edit'></i></button>").click(function()
					{
						...
						renderFileUploadField($("#empImage",$("#editModal")),value.empImage);//*****
					}).tooltip(
					{
						container: tbody
					});
...
function submit(isSilence)
{
	...
	validateFormModal($("#editModal"),function(result){
		if(result){
			...
			form.empImage = getFileUploadJSONField($("#empImage",$("#editModal")));//*****
			form.empType="leader";
			form.contentText=null;
			$.ajax(
			{
				type: method,
