...
{
	var queryParam = $('.form-inline').serialize();
	$.ajax(
	{
		type: "GET",
		url: EnvConfig.LIST_URL(page)+"&"+queryParam,
		...
		success: function(response)
		{
			if (response.code == ResponseCode.Success)
			{
				...
				$.each(response.data, function(index, value)
				{
					...
					var editButton = $("<button class='btn btn-sm btn-default' title='查看'><i class='glyphicon glyphicon-edit'></i></button>").click(function()
					{
						showEditModal("edit",value);
//						renderFileUploadField($("#linkImg",$("#editModal")),value.linkImg);//*****
					}).tooltip(
					{
						container: tbody
					});
				...
				{
					$("#list tbody").html("<tr><td colspan='9' class='text-center active'><h5>列表为空<h5></td></tr>");
				}
