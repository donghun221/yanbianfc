		...
		success: function(response)
		{
			if (response.code == ResponseCode.Success)
			{
				...
				$.each(response.data, function(index, value)
				{
					...
					tr.append("<td>" + value.linkName + "</td>");//*****
					var imageShowBtn = $("<td>").append(getImageShowBtn(EnvConfig.ATTACH_PATH,value.linkImg));
...
function showEditModal(type,data){
	...
	if(type=="edit"){
		...
		renderFileUploadField($("#linkImg"),EnvConfig.ATTACH_PATH,data.linkImg);
	}
...
function submit(isSilence)
{
	...
	validateFormModal($("#editModal"),function(result){
		if(result){
			...
			form.linkImg = getFileUploadJSONField($("#linkImg",$("#editModal")));//*****
			$.ajax(
			{
				type: method,
