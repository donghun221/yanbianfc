		...
		success: function(response)
		{
			if (response.code == ResponseCode.Success)
			{
				...
				$.each(response.data, function(index, value)
				{
					...
					var imageShowBtn = $("<td>").append(getImageShowBtn(EnvConfig.ATTACH_PATH,value.logoImg.fileName));
...
function addImgHTML(initData){
	...
	fileUpload($imgDiv,EnvConfig.ATTACH_UPLOAD_URL,EnvConfig.ATTACH_PATH,function(imgId){
		var subImgInfo  = {id:""};
		subImgInfo.id = $imgDiv.attr("subId");
		subImgInfo.img = {id:imgId};
		submitSub(subImgInfo,function(subId){
...
function submitSub(data,callBack){
	...
	data.prdId = $("#editModal").find("#id[name='id']").val();
	$.ajax(
	{
		type: method,
...
function getListSub(prdId)
{
	...
	$.ajax(
			{
				type: "GET",
				url: EnvConfig.LIST_SUB_URL(prdId),
				contentType: "text/plain; charset=UTF-8",
				dataType: "json",
				success: function(response)
				{
